  ...
  {navLinks.map((link) => (
    <div key={link.name} className="relative group">
      ...
      <div
        onClick={(e) => {
          if (link.hasDropdown) {
            // Dropdown iruntha navigation-a stop panniduvom
            e.preventDefault();
            return; 
          }
          // Dropdown illati mattum navigate aagum
          navigate(link.path);
        }}
        className={`flex items-center font-poppins text-sm font-medium text-gray-700 hover:text-blue-600 transition-colors ${
          link.hasDropdown ? "cursor-default" : "cursor-pointer"
        }`}
      >
        {link.name}
      ...
      {/* Dropdown - Inga "item.path" navigate aagum */}
        ...
        <div className="absolute left-0 mt-7 hidden group-hover:block bg-white shadow-lg rounded-md w-56 before:absolute before:-top-7 before:left-0 before:w-full before:h-7 before:content-['']">
          {ListOfContant[link.path].map((item) => (
            <p
              key={item.Cont}
              onClick={(e) => {
                e.stopPropagation(); // Parent div-oda click trigger aagama thadukkum
                navigate(item.path);
              }}
